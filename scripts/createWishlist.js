...
import { createElement, pluralizeYear } from './helper.js';
...
export const createWishlist = async (pageLogin) => {
    ...
    if (login === pageLogin) {
        const editBtn = createElement('button', {
            ...
            innerHTML: `<svg class="profile__edit-icon" width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    if (user.discription) {
        const discription = createElement('p', {
            className: 'wishlist__discription',
            textContent: user.discription,
        });
    ...
    if (!Object.keys(user.wish).length) {
        ...
        for (const key in user.wish) {
            if (Object.hasOwnProperty.call(user.wish, key)) {
                return
            }
